...
void drawStatusBar()
{
  ...
  if (state.selectButtonPressedSince == 0 || elapsedMillis < BTN_HOLD_LOAD_DELAY)
  {
    ...
    if (state.gpsFix)
    {
      ...
      if (state.gpsFix && state.gpsPrecision < 500)
        u8g2->drawBox(4, 5, 2, 3);
      if (state.gpsFix && state.gpsPrecision < 200)
        u8g2->drawBox(7, 3, 2, 5);
      if (state.gpsFix && state.gpsPrecision < 100)
        u8g2->drawBox(10, 1, 2, 7);
    }
    else
    {
    ...
    if (state.timeHours < 10)
      currentTime += "0";
    currentTime += String(state.timeHours);
    currentTime += ":";

    if (state.timeMinutes < 10)
      currentTime += "0";
