...
void initializeSDCard()
{
  ...
  // If the device was powered off mid-track, close the unfinished GPX file.
  gpxPrefs.begin("gpxtrack", true);
  String unfinishedFile = gpxPrefs.getString("filename", "");
  gpxPrefs.end();
  if (unfinishedFile != "" && SD.exists(unfinishedFile.c_str()))
  {
    File gpxFile = SD.open(unfinishedFile.c_str(), FILE_APPEND);
    if (gpxFile)
    {
      gpxFile.println("</trkseg></trk></gpx>");
      gpxFile.close();
      Serial.println("Repaired unfinished GPX file: " + unfinishedFile);
    }
    gpxPrefs.begin("gpxtrack", false);
    gpxPrefs.remove("filename");
    gpxPrefs.end();
  }
}
...
void saveTrackToGPX()
{
  ...
  if (gpsFixExists && dateIsValid && timeIsValid)
  {
    ...
    if (state.currentGPXFile == "")
    {
      ...
      // Persist the filename so we can repair it on next boot if power is lost.
      gpxPrefs.begin("gpxtrack", false);
      gpxPrefs.putString("filename", state.currentGPXFile.c_str());
      gpxPrefs.end();
      File gpxFile = SD.open(state.currentGPXFile.c_str(), FILE_WRITE);
      if (gpxFile)
      {
        // Write GPX header and opening tags only – NO closing tags.
        // Trackpoints are appended below; closing tags are written by finalizeGPXFile().
    ...
    // Append the new trackpoint – O(1), no seek required.
    File gpxFile = SD.open(state.currentGPXFile.c_str(), FILE_APPEND);
    if (gpxFile)
    {
...
void finalizeGPXFile()
{
  if (state.currentGPXFile != "")
  {
    ...
    // Clear the persisted filename – file is properly closed now.
    gpxPrefs.begin("gpxtrack", false);
    gpxPrefs.remove("filename");
    gpxPrefs.end();
  }
